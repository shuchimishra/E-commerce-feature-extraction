...
def tag_screen_size(pt_text, attr_vals):
    ...
    if 'screen_size' in attr_vals:
        ...
        if len(screen) > 0:
            screen = str(screen[0])
            pattern = re.compile(screen+'\"|'+screen+'[\- ]inch|'+screen+'[\- ]in|'+screen+'inch|'+screen+'in|'+screen+'[ ]inch|'+screen+'[ ]in', re.IGNORECASE)

            # print(pattern)

            for m in pattern.finditer(pt_text):
                print(m.group())
                print(m.start(),m.start()+len(m.group(0)))
...
def tag_screen_resolution(pt_text, attr_vals):
    ...
    for m in pattern.finditer(pt_text):
        output = pt_text[m.start():m.start()+len(m.group(0))]
        print(m.group(0),m.start(),m.start()+m.group(0))
        break
